...
#define P pair<int, int>
    ...
    int secondMinimum(int n, vector<vector<int>>& edges, int time, int change) {
        ...
        while (!pq.empty()) {
            ...
            if (d2[n] != INT_MAX &&
                node ==
                    n) { // We reached n 2nd time means it's the second minimum
                return d2[n];
            }
            ...
            for (auto& nbr : adj[node]) {
                if (d1[nbr] >
                    timePassed + time) { //+time for this edge to reach nbr
                    d2[nbr] = d1[nbr];
                    d1[nbr] = timePassed + time;
                    pq.push({timePassed + time, nbr});
                } else if (d2[nbr] > timePassed + time &&
                           d1[nbr] != timePassed + time) {
